...
export function ThreeCardFan({ onOpenLesson, onOpenPractice }: { onOpenLesson: () => void; onOpenPractice: () => void }) {
  return (
    <section className="relative overflow-hidden bg-[#faf7f2] px-5 py-20 text-[#24161b] sm:px-8 md:px-10 md:py-28">
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="grid gap-12 lg:grid-cols-[.72fr_1.28fr] lg:items-center">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .3 }} transition={{ duration: .55 }}>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#7b001c]/12 bg-white px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[.18em] text-[#7b001c] shadow-sm">One lesson. Three useful shapes.</div>
            <h2 className="mt-5 max-w-xl text-4xl font-semibold leading-[.95] tracking-[-.055em] sm:text-5xl md:text-6xl">
              The same idea can look <span className="text-[#b4002a]">different</span> when it needs to.
            </h2>
            <p className="mt-5 max-w-xl text-base leading-7 text-[#5f6066] sm:text-lg">VisuaLearn keeps the academic content intact — it changes the representation so the learner can choose the form that makes the concept easiest to understand.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <button type="button" onClick={onOpenLesson} className="group inline-flex items-center gap-2 rounded-full bg-[#7b001c] px-5 py-3 text-sm font-semibold text-white shadow-[0_16px_34px_rgba(123,0,28,.18)] transition duration-300 hover:-translate-y-1 hover:bg-[#980022]">Explore the lesson <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></button>
              <button type="button" onClick={onOpenPractice} className="inline-flex items-center gap-2 rounded-full border border-[#7b001c]/15 bg-white px-5 py-3 text-sm font-semibold text-[#5a2630] transition duration-300 hover:-translate-y-1 hover:border-[#b4002a]/30 hover:shadow-[0_14px_30px_rgba(90,0,20,.08)]">Try practice <CircleHelp size={16} /></button>
            </div>
          </motion.div>
                ...
                return (
                  ...
                  >
                    <div className="relative aspect-[4/5] overflow-hidden bg-[#eee7df]">
                      <img src={card.image} alt="" className="h-full w-full object-cover transition duration-700 ease-out hover:scale-105" loading="lazy" />
                      <div className="absolute inset-0 bg-gradient-to-t from-[#1c050b]/65 via-transparent to-transparent" />
                      <div className="absolute left-4 top-4 rounded-full border border-white/25 bg-black/25 px-3 py-1.5 text-[9px] font-semibold tracking-[.16em] text-white backdrop-blur-md">{card.eyebrow}</div>
                    </div>
                    <div className="p-5 sm:p-6">
                      <div className="grid h-9 w-9 place-items-center rounded-xl bg-[#fff0f3] text-[#7b001c]"><Icon size={17} /></div>
                      <h3 className="mt-4 text-xl font-semibold tracking-[-.035em] sm:text-2xl">{card.title}</h3>
