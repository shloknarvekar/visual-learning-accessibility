...
import { normalizeLesson } from '@/lib/mockLesson';
import { createLessonFromFrontend, isLiveApiConfigured, type LessonMode } from '@/lib/api';
...
export default function VisualLearn() {
  const [lesson, setLesson] = useState(() => normalizeLesson());
  ...
  const [lessonMode, setLessonMode] = useState<LessonMode>(isLiveApiConfigured() ? 'live' : 'demo');
  ...
  const progress = Math.round(((question + (answer !== null ? 1 : 0)) / lesson.quiz.length) * 100);
  ...
  const reset = () => {
    ...
    setError('');
    setStage(0);
    setLesson(normalizeLesson());
    setLessonMode(isLiveApiConfigured() ? 'live' : 'demo');
    setApiMessage('');
    setApiPending(false);
  };
  ...
  const begin = async () => {
    ...
    setApiPending(true);
    try {
      const result = await createLessonFromFrontend({ source, url: source === 'youtube' ? url.trim() : undefined, file });
      if (result.mode) setLessonMode(result.mode);
      if (result.message) setApiMessage(result.message);
      if (result.lesson && typeof result.lesson === 'object') {
        setLesson(normalizeLesson(result.lesson as typeof lesson));
      }
    } catch (apiError) {
      setLessonMode('fallback');
      setApiMessage(apiError instanceof Error ? apiError.message : 'The live lesson service is unavailable.');
    } finally {
      setApiPending(false);
    }
  };
  ...
  const choose = (index: number) => {
    ...
    setAnswer(index);
    if (index === lesson.quiz[question].answer) setScore((value) => value + 1);
  };
    ...
    <div className={`min-h-screen ${pageTone} ${accessibilityClass}`} data-view={view}>
      ...
      <main>
        <AnimatePresence mode="wait">
          ...
          {view === 'lesson' && <LessonView lesson={lesson} current={current} activeSection={activeSection} setActiveSection={setActiveSection} lessonMode={lessonMode} highContrast={highContrast} setHighContrast={setHighContrast} goHome={goHome} onQuiz={() => navigate('quiz')} />}
...
function LessonView({ lesson, current, activeSection, setActiveSection, lessonMode, highContrast, setHighContrast, goHome, onQuiz }: any) {
  return <motion.div key="lesson" {...pageFade} className="min-h-screen bg-[#faf7f2] text-[#24161b]"><section className="px-5 pb-10 pt-32 sm:px-8 md:px-10"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.82fr_1.18fr] lg:items-end"><div><div className="text-[10px] font-semibold uppercase tracking-[.2em] text-[#7b001c]">03 · Learn</div><RotatingHeadline prefix="Learn in a form that feels" words={['natural.','clear.','visual.','yours.']}/><p className="mt-5 max-w-2xl text-base leading-7 text-[#5f6066] sm:text-lg">Switch sections without losing the thread. The left rail controls pacing; content and visualization stay independent.</p><div className="mt-7 flex flex-wrap gap-2"><span className="rounded-full border border-[#7b001c]/12 bg-white px-3 py-2 text-xs">Section {String(activeSection+1).padStart(2,'0')}</span><span className="rounded-full border border-[#7b001c]/12 bg-white px-3 py-2 text-xs">{lessonMode==='live'?'LIVE':lessonMode==='fallback'?'FALLBACK':'DEMO'}</span></div></div><MiniFan labels={['MAP','PROCESS','EXAMPLE']}/></div></section><section className="px-5 pb-16 sm:px-8 md:px-10 md:pb-20"><div className="mx-auto grid max-w-7xl gap-5 lg:grid-cols-[230px_minmax(0,1fr)_260px]"><aside className="rounded-[26px] border border-[#7b001c]/12 bg-white p-4 shadow-[0_18px_50px_rgba(70,0,18,.06)]"><div className="mb-3 flex items-center gap-2 font-semibold"><BookOpen size={17}/> Lesson map</div><div className="grid gap-1">{lesson.sections.map((section:any,index:number)=><motion.button whileHover={{x:3}} key={section.id} onClick={()=>setActiveSection(index)} className={`focus-ring rounded-xl px-3 py-3 text-left text-sm transition ${activeSection===index?'bg-[#fff0f3] font-semibold text-[#7b001c]':'text-[#5f6066] hover:bg-[#faf7f2]'}`}>{String(index+1).padStart(2,'0')} · {section.title}</motion.button>)}</div></aside><article className="min-w-0 rounded-[30px] border border-[#7b001c]/12 bg-white p-6 shadow-[0_26px_80px_rgba(70,0,18,.07)] md:p-9"><AnimatePresence mode="wait"><motion.div key={current.id} initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} transition={{duration:.42,ease}}><div className="flex flex-wrap items-center justify-between gap-2"><span className="rounded-full bg-[#fff0f3] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[.18em] text-[#7b001c]">{current.type}</span><span className="text-xs text-[#5f6066]">Person 3 renderer boundary</span></div><h2 className="mt-5 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">{current.title}</h2><p className="mt-4 max-w-3xl text-lg leading-8 text-[#5f6066]">{current.summary} {current.content}</p>{current.bullets&&<div className="mt-8 grid gap-3 sm:grid-cols-2">{current.bullets.map((bullet:string,i:number)=><motion.div key={bullet} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:i*.06}} className="rounded-2xl border border-[#7b001c]/10 bg-[#faf7f2] p-4 text-sm leading-6"><Check className="mb-2 text-[#b4002a]" size={17}/>{bullet}</motion.div>)}</div>}<Visualization kind={current.visualization?.kind||'concept'} label={current.visualization?.label||'Visualization'} /><div className="mt-8 flex flex-wrap justify-between gap-3 border-t border-[#7b001c]/10 pt-5"><button disabled={activeSection===0} onClick={()=>setActiveSection((value:number)=>Math.max(0,value-1))} className="focus-ring rounded-full border border-[#7b001c]/12 px-5 py-3 text-sm disabled:opacity-40">Previous</button>{activeSection<lesson.sections.length-1?<button onClick={()=>setActiveSection((value:number)=>value+1)} className="focus-ring inline-flex items-center gap-2 rounded-full bg-[#7b001c] px-5 py-3 text-sm font-semibold text-white">Next section <ArrowRight size={15}/></button>:<button onClick={onQuiz} className="focus-ring inline-flex items-center gap-2 rounded-full bg-[#b4002a] px-5 py-3 text-sm font-semibold text-white">Start practice <ArrowRight size={15}/></button>}</div></motion.div></AnimatePresence></article><aside className="rounded-[26px] border border-[#7b001c]/12 bg-[#130307] p-5 text-white"><div className="flex items-center gap-2 font-semibold"><MousePointer2 size={17}/> Learn your way</div><p className="mt-2 text-sm leading-6 text-white/50">Control pacing, change representation and pause when you need another shape.</p><div className="mt-5 rounded-2xl border border-white/10 bg-white/[.04] p-4"><div className="text-[10px] uppercase tracking-[.18em] text-white/35">Current representation</div><div className="mt-2 text-lg font-semibold">{current.visualization?.label}</div><div className="mt-2 text-xs text-white/40">Ready for renderer injection.</div></div><div className="mt-3 rounded-2xl border border-white/10 bg-white/[.04] p-4"><div className="text-[10px] uppercase tracking-[.18em] text-white/35">Accessibility</div><button type="button" onClick={()=>setHighContrast((value:boolean)=>!value)} className="focus-ring mt-3 w-full rounded-xl border border-white/10 bg-white/[.03] px-3 py-2.5 text-sm text-white transition hover:border-[#ff5278]/40 hover:bg-[#b4002a]/10">{highContrast?'Standard contrast':'High contrast'}</button></div></aside></div></section><SignalBand title="One lesson. Different representations." text="Switch from a map to a process, from a process to an example, or from content to practice without losing your place."><MiniFan labels={['MAP','PROCESS','EXAMPLE']} dark/></SignalBand><CompactFooter onBackHome={goHome}/></motion.div>;
}
              ...
              <motion.div key={question} initial={{ opacity: 0, x: 24, filter: 'blur(5px)' }} animate={{ opacity: 1, x: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, x: -24, filter: 'blur(5px)' }} transition={{ duration: .42, ease }}>
                  ...
                  {item.options.map((option: string, index: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="flex-1 text-[16px] font-medium text-white sm:text-[17px]">{option}</span>
                ...
                {answer !== null && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-6 rounded-2xl border border-[#ff5278]/25 bg-[#7b001c]/18 p-5">
                    <div className="text-base font-semibold text-white">{answer === item.answer ? 'Correct.' : 'Here’s the idea.'}</div>
