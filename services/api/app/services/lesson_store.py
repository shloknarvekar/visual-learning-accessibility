...
Callers depend on the `LessonStore` interface. `FileLessonStore` keeps each lesson record as a
JSON file on local disk, which is enough until the workflow is proven; a database-backed store can
...
class FileLessonStore:
    ...
    def save(self, record: LessonRecord) -> None:
        ...
        path = self._path_for(record.lesson_id)
        temporary = path.with_suffix(".json.tmp")
        temporary.write_text(record.model_dump_json(exclude_none=True, indent=2), encoding="utf-8")
