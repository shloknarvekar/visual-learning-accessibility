...
export function RotatingHeadline({ prefix, words }: { prefix: string; words: string[] }) {
...
export function MiniFan({
  ...
  const fills = dark
    ? ["bg-[#2d0811]", "bg-[#4a0b19]", "bg-[#160509]"]
    : ["bg-[#fff0f3]", "bg-white", "bg-[#f4ebe9]"];
  return (
    <div className="relative mx-auto flex w-full max-w-2xl items-end justify-center px-4 py-8">
      {labels.slice(0, 3).map((label, i) => (
        ...
        >
          <div
            className={`text-[9px] font-semibold tracking-[.2em] ${dark ? "text-[#ff5278]/80" : "text-[#7b001c]/55"}`}
          >
            0{i + 1}
          </div>
          <div
            className={`mt-20 text-xl font-semibold tracking-tight sm:mt-28 sm:text-2xl ${dark ? "text-white" : "text-[#24161b]"}`}
          >
            {label}
          </div>
          <div className={`mt-2 text-xs leading-5 ${dark ? "text-white/45" : "text-[#5f6066]"}`}>
            One idea. One useful representation.
          </div>
          <div
            className={`absolute inset-x-4 bottom-4 h-px ${dark ? "bg-white/10" : "bg-[#7b001c]/10"}`}
          />
        </motion.div>
      ))}
    </div>
...
export function SignalBand({
  ...
  return (
    <section
      className={`${dark ? "bg-[#130307] text-white" : "bg-[#faf7f2] text-[#24161b]"} relative overflow-hidden border-y ${dark ? "border-white/10" : "border-[#7b001c]/10"}`}
    >
      ...
      <div className="relative mx-auto grid max-w-7xl gap-8 px-5 py-16 sm:px-8 md:grid-cols-[.72fr_1.28fr] md:items-center md:px-10 md:py-20">
        <div>
          <div
            className={`text-[10px] font-semibold uppercase tracking-[.2em] ${dark ? "text-[#ff5278]" : "text-[#7b001c]"}`}
          >
            One lesson. Many ways to click.
          </div>
            ...
            {(() => {
              ...
              return (
                <>
                  {parts.join(" ")}{" "}
                  {dark ? (
                    <span className="text-[#ff5278]">{accent}</span>
                  ) : (
                    <span className="text-[#b4002a]">{accent}</span>
                  )}
                </>
              );
            })()}
          ...
          <p
            className={`mt-5 max-w-xl text-base leading-7 sm:text-lg ${dark ? "text-white/55" : "text-[#5f6066]"}`}
          >
            {text}
          </p>
        </div>
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="flex flex-wrap items-center justify-between gap-5 border-b border-white/10 pb-8">
          <div>
            ...
            <div className="mt-2 text-3xl font-semibold tracking-[-.04em] sm:text-4xl">
          ...
          <motion.button
            ...
            whileHover={{ y: -3 }}
            whileTap={{ scale: 0.97 }}
            className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[.04] px-5 py-3 text-sm font-semibold text-white transition hover:border-[#ff5278]/50 hover:bg-[#b4002a]/15"
          >
