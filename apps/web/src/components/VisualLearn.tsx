    ...
    <div className={`min-h-screen ${pageTone} ${accessibilityClass}`} data-view={view}>
      ...
      <main>
        <AnimatePresence mode="wait">
          ...
          {view === 'lesson' && <LessonView lesson={lesson} current={current} activeSection={activeSection} setActiveSection={setActiveSection} lessonMode={lessonMode} highContrast={highContrast} setHighContrast={setHighContrast} goHome={goHome} onQuiz={() => navigate('quiz')} lessonId={lessonId} onReload={reloadLesson} />}
          ...
          <div className="grid gap-6 p-2 pt-6 lg:grid-cols-[.9fr_1.1fr] lg:p-4">
            <div className="rounded-[26px] bg-[#130307] p-6 text-white md:p-8">
              ...
              <div className="mt-7 grid grid-cols-3 gap-2 rounded-2xl border border-white/10 bg-white/[.03] p-1">
                ...
                <button type="button" onClick={() => { setSource('video'); setError(''); }} className={`rounded-xl px-4 py-3 text-sm font-semibold transition ${source === 'video' ? 'bg-white text-[#160509]' : 'text-white/55 hover:text-white'}`}><Play className="mr-2 inline" size={16} />Video</button>
              </div>
              <div className="mt-6 grid gap-2 text-sm text-white/50">
                <div className="flex items-center gap-3"><Check className="text-[#ff5278]" size={15} /> Connected to the real lesson service</div>
              ...
              <AnimatePresence mode="wait">
                {source === 'youtube' ? (
                  <motion.div key="youtube" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }} transition={{ duration: .4, ease }}>
                    ...
                    <p className="mt-2 text-xs text-[#5f6066]">Paste a link to a single public YouTube video.</p>
                  </motion.div>
                ) : source === 'pdf' ? (
                  <motion.div key="pdf" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }} transition={{ duration: .4, ease }}>
                    ...
                    <button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => {
                      ...
                      <div className="mt-2 text-sm text-[#5f6066]">PDF · uploaded to the lesson service</div>
                    </button>
                  </motion.div>
                ) : (
                  <motion.div key="video" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }} transition={{ duration: .4, ease }}>
                    <input ref={inputRef} type="file" accept="video/mp4,video/mpeg,video/mov,video/quicktime,video/avi,video/x-msvideo,video/webm,video/wmv,video/x-ms-wmv,video/3gpp,.mp4,.mov,.avi,.webm,.wmv,.3gp,.mpeg,.mpg" className="sr-only" onChange={(event) => {
                      const selected = event.target.files?.[0] || null;
                      setError('');
                      if (selected && !selected.type.startsWith('video/') && !/\.(mp4|mov|avi|webm|wmv|3gp|mpeg|mpg)$/i.test(selected.name)) {
                    ...
                    <button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => {
                      event.preventDefault();
                      const dropped = event.dataTransfer.files?.[0];
                      if (dropped?.type.startsWith('video/') || /\.(mp4|mov|avi|webm|wmv|3gp|mpeg|mpg)$/i.test(dropped?.name ?? '')) {
                        setFile(dropped ?? null);
                        setError('');
                      } else setError('Please drop a video file.');
                    }} className="group focus-ring w-full rounded-[26px] border-2 border-dashed border-[#7b001c]/14 bg-white p-10 text-center transition hover:-translate-y-1 hover:border-[#b4002a]/45 hover:shadow-[0_22px_45px_rgba(120,0,28,.08)]">
                      <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#fff0f3] text-[#b4002a] transition group-hover:scale-105"><Play /></div>
                      <div className="mt-4 font-semibold">{file ? file.name : 'Drop a video or choose a file'}</div>
                      <div className="mt-2 text-sm text-[#5f6066]">Video · uploaded to the lesson service</div>
                    </button>
                  </motion.div>
                )}
              </AnimatePresence>
...
function formatSourceReferences(section: Section): string | null {
  ...
  if (timestamps.length > 0) {
    const formatted = timestamps.map((ref) => {
      ...
      return ref.end_time_seconds !== undefined ? `${start}–${formatTimestamp(ref.end_time_seconds)}` : start;
    });
...
function LessonView({ lesson, current, activeSection, setActiveSection, lessonMode, highContrast, setHighContrast, goHome, onQuiz, lessonId, onReload }: LessonViewProps) {
      ...
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.82fr_1.18fr] lg:items-end">
        <div>
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            ...
            {lessonId && <button type="button" onClick={onReload} title="Reload this lesson from the server" className="focus-ring rounded-full border border-[#7b001c]/12 bg-white px-3 py-2 text-xs transition hover:border-[#b4002a]/45">Lesson {lessonId.slice(0,8)}</button>}
          </div>
        </div>
